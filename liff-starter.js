...
function getP(){
    ...
    if (!tipe) {
        ...
        document.getElementById('simgx').addEventListener('click', function () {
            liff.sendMessages([{
                type: 'text',
                text: atob('bGluZTovL2FwcC8xNjAyNjg3MzA4LURnZWRHazlBP3R5cGU9c2ltZyZxPUxpbmUKCnR5cGUgPT4gc2ltZwpxID0+IFVwIHRvIHlvdQ==')
            }]).then(function () {
                liff.closeWindow();
            });
        });
...
function searchImage(){
    ...
    if (tipe === 'simg') {
        ...
        var data = getJSON('https://ari-api.herokuapp.com/images?q='+q).then(function(data) {
        };
        var resultx = data.result;
        var jsonn = JSON.parse(resultx);
        var datax = jsonn.result;
        if (!datax) {
            liff.sendMessages([{
              type: "template",
              altText: "Image "+q+"Not Found",
              template: {
                 type: "image_carousel",
                 columns: [{
                     imageUrl: "https://howfix.net/wp-content/uploads/2018/02/sIaRmaFSMfrw8QJIBAa8mA-article.png",
                     action: {
                         type: "uri",
